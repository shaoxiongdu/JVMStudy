...
《Java虚拟机规范》对这三点的要求并不是特别的具体。因此，留给虚拟机实现与Java应用的灵活度都是很大的。
...
之后会在堆中实例化一个java.lang.Class类型的对象，
...
1. ### 验证(Verify)
   ...
   - `字节码验证`
     ...
     如果一个类型中的方法体没有通过此阶段，那它一定是有问题的。但是，不可以认为只要通过此阶段验证，一定没有问题。通过程序去校验程序的逻辑是无法做到绝对准确的。
