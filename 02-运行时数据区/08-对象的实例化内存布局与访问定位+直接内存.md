...
3. #### 处理并发安全问题
   ...
   2. TLAB把内存分配的动作按照线程划分在不同的空间之中进行，即每个线程在Java堆的伊甸园区预先分配一小块内存，称为本地线程分配缓冲区，（TLAB ，Thread Local Allocation Buffer） 虚拟机是否使用TLAB，可以通过 `-XX:+UseTLAB` 参数来设定。
...
JVM是如何通过栈帧中的对象引用访问到其内部的对象实例呢？-> 定位，通过栈上reference访问
...
- 来源于NIO（1.7之后的new I/O），通过存在堆中的DirectByteBuffer操作本地内存
  ...
  - 通常，访问直接内存的速度会优于Java堆。即读写性能高
    ...
    也可能导致 `OutOfMemoryError: Direct buffer memory` 异常
