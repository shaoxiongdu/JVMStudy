...
用户线程指完全建立在用户态的线程库中，用户线程的创建、同步、销毁、调度等操作完全在用户态中完成，不需要频繁切换内核态，因此速度很快。很多高性能的数据库中的多线程就是用的用户线程。
...
- 由于操作系统只将处理器的资源分配到进程程度，所以`阻塞如何处理`、`多处理器如何分配资源`等问题都需要由用户解决。
...
JDK1.3之后，普遍采用内核线程来实现，也就是 1:1 的线程模型。
