...
从 Java 5 开始，JDK 自带的 Java 监控和管理控制台。

#### 启动

`cmd` 输入 `jconsole` 或者在 jdk `bin` 目录下执行即可。
...
`cmd` 输入 `jvisualvm` 或者在 jdk `bin` 目录下执行即可。
...
>
> IDEA插件: [下载地址](https://plugins.jetbrains.com/files/253/12253/idea-jprofiler.zip?updateId=12253&pluginId=253&family=INTELLIJ)
...
- 实际上，很多时候一些不好的编程习惯会使得某些对象的生命周期变得很长，比如局部变量定义为类变量，类变量定义为静态变量，此时，可能会导致OOM，这种也可以叫做宽泛意义上的"内存泄露"。
- 比如
